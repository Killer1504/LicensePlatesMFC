...
void CRunningDlg::OnBnClickedTabRunningBtnTest()
{
	...
	AfxBeginThread(ImageProcessThread, NULL);


	return;
	//if (pDoc->m_imgOrigin[0].empty()) return;
	//Mat m_imgGray;
	//Mat m_imgThreshold;
	//if (pDoc->m_imgOrigin[0].channels() > 1)
	//	cvtColor(pDoc->m_imgOrigin[0], m_imgGray, COLOR_BGR2GRAY);
	//else
	//	m_imgGray = pDoc->m_imgOrigin[0].clone();

	//cv::normalize(m_imgGray, m_imgGray, 0, 255, CV_MINMAX);
	////pView->DisplayImage(m_imgGray);
	...
	//pView->DisplayImage(m_imgThreshold);
